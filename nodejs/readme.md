...
// 一个用Node编写的web服务器的例子，它的响应是“Hello World”。
// 要运行服务器，将代码放入名为example的文件中。并使用node程序执行它。
...
// 在浏览器中，顶级作用域是全局作用域。
// 这意味着在浏览器中，如果你在全局变量var中，就会定义一个全局变量。
// 在Node中，这是不同的。顶层范围不是全局范围; Node模块中的某些东西将是该模块的本地变量。

__filename;  // 正在执行的代码的文件名。(绝对路径)
__dirname;   // 当前执行脚本所在目录的名称。(绝对路径)
module;      // 对当前模块的引用。特别是 module.exports 用于定义模块导出和提供内容，通过 require()
exports;     // 相当于 module.exports
process;     // process是一个全局对象, 可以从任何地方访问。它是事件发射器的一个实例。
Buffer;      // 缓冲区类是直接处理二进制数据的全局类型。
```
...
```
setTimeout(callback, delay, [arg], [...]);   // 延迟执行
clearTimeout(t);                             // 清除之前定义的 setTimeout
setInterval(callback, delay, [arg], [...]);  // 间隔执行
clearInterval(t);                            // 清除之前定义的 setInterval
setImmediate(callback, [arg], [...]);        // 将事件插入到事件队列尾部，主线程和事件队列的函数执行完成之后立即执行，优先于 setTimeout 和 setInterval
clearImmediate(immediateObject);             // 清除之前定义的 setImmediate
...
```
var module = require('./module.js');    // 在同一目录中加载模块 module.js
